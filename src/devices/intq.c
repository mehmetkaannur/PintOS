...
static void intq_wait (struct intq *q, struct thread **waiter);
...
static void
intq_wait (struct intq *q UNUSED, struct thread **waiter) 
{
  ASSERT (!intr_context ());
  ASSERT (intr_get_level () == INTR_OFF);
